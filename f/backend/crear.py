...
def main(
    ...
    # Guardamos quién creó la operación dentro de datos_extra
    ...
    try:
        result = db.query(
            "INSERT INTO operaciones_ok (id, nombre_solicitante, monto, producto, fecha_operacion, estado, etapa_actual, aprobaciones, documentos, pendientes, datos_extra, fecha_creacion) VALUES ($1, $2, $3, $4, CAST($5 AS DATE), $6, $7, $8::jsonb, $9::jsonb, $10::jsonb, $11::jsonb, NOW()) RETURNING id, nombre_solicitante, monto, producto, fecha_operacion, estado, etapa_actual, fecha_creacion",
            nuevo_id, nombre_solicitante.strip(), float(monto), producto, fecha_operacion, 'activa', 1, '[]', '[]', '[]', json.dumps(datos_iniciales)
        ).fetch_one()
